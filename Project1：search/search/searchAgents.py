...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        visited_corners = ()  # self.corners是元组，这里也用元组存储访问过的角落
        # 若起始位置是角落，加入访问角落列表
        if self.startingPosition in self.corners:
            visited_corners += (self.startingPosition,)  # 元组连接
    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                new_visited_corners = state[1]
                next_position = (nextx, nexty)
                # 若下一步是角落且当前未访问过，则更新visit_corners
                if (next_position in self.corners) and not (next_position in new_visited_corners):
                    new_visited_corners += (next_position,)
                # 题目要求cost为1
                successor = ((next_position, new_visited_corners), action, 1)
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    for corner in corners:
        if corner not in visited_corners:
            no_vis_corners.append(corner)

    total_dist = 0

    while no_vis_corners:
        dist = 99999
        for corner in no_vis_corners:
            if util.manhattanDistance(now_state, corner) < dist:
                dist = util.manhattanDistance(now_state, corner)
                tmp_corner = corner
        total_dist += dist
        now_state = tmp_corner
        no_vis_corners.remove(now_state)
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    for food_position in food_list:
        total_dist = max(mazeDistance(
            now_position, food_position, problem.startingGameState), total_dist)
    return total_dist

    # while food_list:
    #     dist = 99999
    #     for food_position in food_list:
    #         tmp_dist = mazeDistance(
    #             now_position, food_position, problem.startingGameState)
    #         if tmp_dist < dist:
    #             dist = tmp_dist
    #             tmp_position = food_position
    #     total_dist += dist
    #     now_position = tmp_position
    #     food_list.remove(now_position)
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState: pacman.GameState):
        ...
        "*** YOUR CODE HERE ***"
        if problem.isGoalState(startPosition) == True:
            return []
        ...
        for position, action, cost in problem.getSuccessors(startPosition):
            # 用cost做priority
